...
void Process::broadcastValue(VALUE val,SEQUENCE_NUM seq) {
    ...
    if (it != this->Recho.end()) {
        ...
        l.aggregateSign(KF,this->priv,this->selfid,PHASE_ECHO,this->selfid,seq,val,stats[selfid]);
...
void Process::finish() {
    ...
    if (CLEAR) {
        ...
        for (int i = 0; i < nGates; i++) { outQueues[i].clear(); }
    ...
    if (selfid < nProcesses - numByz) {
        ...
        if (numFinish == nProcesses - numByz) {
            ...
            for (int i = 0; i < nProcesses; i++) {
                std::cout << i << " : crypto time (microsec) = " << stats[i]
                        << endl;
...
void Process::triggerDeliverIfActive(PROCESS_ID pid, SEQUENCE_NUM seq, VALUE val) {
    ...
    cryptoTimesDeliver[pid] = stats[pid].getCryptoTime();
...
void Process::deliverMessage(PID sender, PROCESS_ID pid, SEQUENCE_NUM seq, VALUE val, Aggregate esigns, Aggregate signs) {
    ...
    if (it1 != this->Rdeliver.end()) {
        ...
        if (it2 != m.end()) {
            ...
            signs.aggregateSign(KF,this->priv,this->selfid,PHASE_DELIVER,pid,seq,val,stats[selfid]);//,esigns);
            //printSigns(signs);
...
void Process::handleBroadcast(PID sender, Message bcast) {
    ...
    if (it != this->Recho.end()) {
        ...
        sig.aggregateSign(KF,this->priv,this->selfid,PHASE_ECHO,pid,seq,val,stats[selfid]);
...
void Process::handleEcho(PID sender, Message echo) {
    ...
    if (it1 != this->Recho.end()) {
        ...
        if (it2 != m.end()) {
            ...
            signs.aggregateSign(KF,this->priv,this->selfid,PHASE_ECHO,pid,seq,val,stats[selfid]);
            sig.aggregateWith(KF,signs,stats[selfid]);
            ...
            if (sig.numSigners() > 2 * maxF && oldSize <= 2 * maxF) { // i.e., for the 1st time
...
void Process::handleDeliver(PID sender, Message del) {
    ...
    if (it1 != this->Rdeliver.end()) {
        ...
        if (it2 != m.end()) {
            ...
            aggr1.aggregateWith(KF,sig,stats[selfid]);
...
//                if (DEBUG) { std::cout << "queue is empty channelId = " << channelId << "; processToGate[channelId] = " << processToGate[channelId] << endl; }
                ...
                if (!gate("gate$o", channelId)->getTransmissionChannel()->isBusy()) { // channel is not busy
                    ...
                    if (!channelTimers[channelId]->isScheduled()) {
                        scheduleAt(gate("gate$o", channelId)->getTransmissionChannel()->getTransmissionFinishTime(), channelTimers[channelId]);
                    }
